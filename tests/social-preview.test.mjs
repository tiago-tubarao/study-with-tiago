...
test('generic share cards use the Study with Tiago banner social preview', () => {
  genericPages.forEach((page) => {
    ...
    assert.match(html, /assets\/og-study-with-tiago-banner\.png\?v=2/, `${page} should use the banner social preview`);
    assert.doesNotMatch(html, /assets\/og-study-with-tiago-clean\.png\?v=1/, `${page} should not use the clean social preview`);
